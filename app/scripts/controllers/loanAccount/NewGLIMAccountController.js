    ...
    mifosX.controllers = _.extend(module, {
        NewGLIMAccountController: function (scope, rootScope, routeParams, resourceFactory, location, dateFilter, WizardHandler) {
            ...
            scope.submit = function () {
                ...
                // to uniquely represent and link child applications of GLIM application
                //when combined with is isAccepting application
                //makes it robust against duplicates
                ...
                for (var i in scope.group.clients) {
                    ...
                    if( scope.group.clients[i].isSelected ){
                        ...
                        /*   if (scope.charges.length > 0) {
                         ...
                         for (var i in scope.charges) {
                         loanApplication.charges.push({ chargeId: scope.charges[i].chargeId, amount: scope.charges[i].amount, dueDate: dateFilter(scope.charges[i].dueDate, scope.df) });
                         }
                         }*/
                        ...
                        /*  if(scope.formData.repaymentFrequencyNthDayType.id)
                         {
                         loanApplication.repaymentFrequencyNthDayType=scope.formData.repaymentFrequencyNthDayType.id;
                         }*/

                        //  loanApplication.repaymentFrequencyDayOfWeekType=scope.formData.repaymentFrequencyDayOfWeekType.id;
                        ...
                        //   loanApplication.charges = [];
                        //  loanApplication.charges= scope.charges;
